...
export async function writeToFiles(filesAndFolders, parentFolders) {
  parentFolders = parentFolders || "";

  filesAndFolders.files.forEach((file) => {
    const filePath = `src/content/googleDocPages/${parentFolders}${file.name}.md`;
    fs.mkdirSync(path.dirname(filePath), { recursive: true });
    fs.writeFileSync(filePath, file.markdown);
  });
  filesAndFolders.folders.forEach((folder) => {
    writeToFiles(folder, `${parentFolders}${folder.name}/`);
  });
}
